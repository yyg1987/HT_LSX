...
namespace LY.PF.Tests
{
    ...
    /// Provides methods to easily work with <see cref="PFDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<PFTestModule>
    {
        ...
        protected void UsingDbContext(Action<PFDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PFDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PFDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PFDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PFDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PFDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PFDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PFDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PFDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PFDbContext, Task<T>> func)
        {
            T result;
